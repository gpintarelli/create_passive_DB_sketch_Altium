...
for res_scale_rage in range(num_decades-1):
    for loop_res_value in range (len(E96_RES_TABLE)):
        ...
        cursor.execute("insert into Resistor_SMD([ID],[Description],[Comment],[Library Ref],[Footprint Ref 1],[Footprint Ref 2],[Footprint Ref 3]) values(?, ?, ?, ?, ?, ?, ?)", (id_DB, make_description, res_value, lib_ref, foot1_ref, foot2_ref, foot3_ref))
        id_DB=id_DB+1
cursor.commit()
